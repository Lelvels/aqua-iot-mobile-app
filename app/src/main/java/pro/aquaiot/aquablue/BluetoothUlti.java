...
public class BluetoothUlti {
    static String TAG = "BluetoothUlti";
    ...
    static int compareTo(BluetoothDevice a, BluetoothDevice b){
        try{
            ...
            Log.e(TAG, "Error compareTo() ,msg: " + e.getMessage());
            return -1;
        }
    }
    ...
    private static void showSettingsDialog(Fragment fragment) {
        String s = fragment.getResources().getString(fragment.getResources()
    ...
    /**
     * Check if the app having permission, if not then create show a dialog to connect to bluetooth,
     * only apply for device with version more than 12.
     * @param fragment : fragment of the
        ...
        if(missingPermissions){
            if(showRationale){
                showRationaleDialog(fragment, (dialog, which) -> {
                    //TODO: what is ActivityResultLauncher, do this launch in bluetooth?
    ...
    static void onPermissionResult(Fragment fragment, boolean granted, PermissionGrantedCallback cb){
        if(Build.VERSION.SDK_INT < Build.VERSION_CODES.S)
            return;
        boolean showRationale = fragment.shouldShowRequestPermissionRationale(Manifest.permission.BLUETOOTH_CONNECT);
        if(granted){
